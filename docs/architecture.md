...
**Recommendation in one line:** a stateless TypeScript Cloudflare Worker at `https://sql.fm/mcp`,
built on MCP SDK v2 and Cloudflare's `createMcpHandler`, reading a versioned `dataset.v1.json`
generated by the *existing SQL.FM build* and bundled into the Worker at deploy time.
...
3. **The hosting decision is nearly free.** SQL.FM is on Cloudflare. A Worker on the same zone
   deploys from the same pipeline, serves from a first-party `sql.fm` URL, costs effectively
   nothing at this traffic, and requires no new infrastructure for the owner to operate.
...
| Runtime / hosting | **Cloudflare Workers**, via `createMcpHandler` from `agents/mcp/server` |
...
flowchart TB
    ...
    subgraph worker["Cloudflare Worker — sql.fm/mcp"]
...
| **Transport** | `POST /mcp` with `Mcp-Method` / `Mcp-Name`; `server/discover`; `tools/list` shape, deterministic order, `ttlMs` + `cacheScope`; missing-header rejection; CORS; method-not-allowed; oversized body; `GET /health` |
...
│
├── src/
│   ├── index.ts                   # Worker entry: /mcp + /health
...
| **5** | MCP surface: `createMcpHandler` at `/mcp`, three tools, three resources, annotations, `outputSchema` + `structuredContent`, error conventions, structured logging | 1 d | Transport + e2e tests pass; MCP Inspector connects |
...
| Option | Assessment |
|---|---|
| **1. Cloudflare Worker alongside SQL.FM** | ✅ **Chosen.** Same platform, same pipeline, first-party URL, stateless matches `2026-07-28`, near-zero cost and operations, free WAF/rate limiting |
| **2. Standalone TypeScript service** (Node + Express/Fastify, container) | Rejected: same code, strictly more operations — a host, TLS, patching, monitoring, scaling, and a bill. Its only real advantage is portability, which the provider abstraction already supplies (the domain core is runtime-agnostic; a Node adapter is a small file if it is ever needed). Adds burden to a solo maintainer for no capability gain |
...
|---|---|---|
| Dataset refresh (first-party) | Every SQL.FM data change | **Zero** — one step in the existing build |
